  ...
  return (
    <div className="max-w-5xl mx-auto space-y-6 pb-12 pt-6 px-4">
      <div className="flex items-center justify-between mb-8">
        <div>
          <h1 className="text-2xl font-bold text-slate-900 dark:text-white">حالاتي (المرضى)</h1>
          <p className="text-slate-500 dark:text-slate-400 mt-1">إدارة الطلبات المقبولة والتواصل مع المرضى</p>
        </div>
      </div>
      ...
      {appointments.length === 0 ? (
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
          {appointments.map((appt) => (
            <div key={appt.id} className="bg-white dark:bg-slate-900 rounded-3xl p-6 border border-slate-200 dark:border-slate-800 shadow-sm flex flex-col transition-colors hover:border-teal-500/30">
              ...
              <div className="bg-slate-50 dark:bg-slate-800/50 rounded-2xl p-4 mb-6 flex-1">
              ...
              <div className="grid grid-cols-2 gap-3 mt-auto">
                ...
                {appt.status !== 'completed' ? (
                  <button
                    ...
                    className="flex items-center justify-center gap-2 bg-teal-600 hover:bg-teal-700 text-white py-3 rounded-xl font-bold transition-colors"
                  >
                    <FileText className="w-4 h-4" /> إنهاء التقرير
                  </button>
                ) : (
                  <div className="flex items-center justify-center gap-2 bg-green-50 text-green-600 py-3 rounded-xl font-bold border border-green-100">
